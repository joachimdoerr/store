...
';
    ...
    public static function addPluginSiteHeadlinesToConfig(rex_addon_interface $addon, $pluginKey = null)
    {
        ...
        if (is_null($pluginKey)) {
            ...
            $navigationProvider = new StoreNavigationProvider($addon->getName());
            if ($navigationProvider->existDefinitions()) {
                foreach ($navigationProvider->getDefinition()[0]->getDefinitions() as $key => $main) {
                    foreach ($main as $k => $page) {
                        if (is_array($page)) {
                            foreach ($page as $ik => $item) {
                                ...
                                } else if (isset($item['url_parameter']['store_path']) && strpos($item['url_parameter']['store_path'], $pluginKey) !== false) {
                                    $plugin->setConfig('headline_' . $item['url_parameter']['store_path'], $addon->getName() . '_' . $item['name'] . '_master_headline');
                                    $plugin->setConfig('subheadline_' . $item['url_parameter']['store_path'], $addon->getName() . '_' . $item['name'] . '_master_description');
                                    $plugin->setConfig('icon_' . $item['url_parameter']['store_path'], $item['icon']);
                                }
                            }
                        }
                    }
