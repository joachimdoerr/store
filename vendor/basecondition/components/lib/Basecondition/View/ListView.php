...
class ListView
{
    ...
    public function __construct($addonKey, $searchFile, $rows = 30, $debug = false, $initList = true, array $urlParameters = array())
    {
        ...
        $searchPath = $addon->getDataPath('resources');

//        dump($searchPath);
//        dump(sprintf(self::SEARCH_SCHEMA, $this->searchFile));

        $definition = DefinitionProvider::load(sprintf(self::SEARCH_SCHEMA, $this->searchFile), $searchPath, $addon->getCachePath(), new DatabaseDefinitionMergeHandler, true);
//        if (isset($definition['data']) && is_array($definition['data']))
...
//        }
        ...
        if (isset($this->definition['data']) && is_array($this->definition['data']))
            foreach ($this->definition['data'] as $key => $value) {
        ...
        foreach ($this->items as $item) {
            if (array_key_exists('list_prio', $item)) {
                ViewHelper::arraySortByColumn($this->items, 'list_prio');
                break;
            }
        }
...
//        echo '</pre>';
    ...
    public function createList($query = null, $rows = null, $debug = null)
    {
        ...
        $this->list = rex_list::factory($query, $rows, $this->searchFile, $debug);
...
//        }
        ...
        if (is_array($this->items)) {
            foreach ($this->items as $item) {
                ...
                if (!array_key_exists('callable', $item)) {
                    $select[] = $this->tableKey . '.' . $item['name'];
                } else {
                    if ($item['status'] == 1) {
                        $select[] = $this->tableKey . '.status';
                    }
                }
            }
        }
    ...
    public function createQuery()
    {
        ...
        $this->query = 'SELECT ' . implode(', ', $this->selects) . ' FROM ' . $this->tableKey . ' AS ' . $this->tableKey . ' ' . $this->where . ' ' . $this->orderBy;
        return $this->query;
    }
    ...
    public function addIdElement($parameter = array(), $icon = 'fa-file-text-o')
    {
        ...
        $thIcon = '<a href="' . $this->list->getUrl(['func' => 'add']) . '" title=""><i class="rex-icon rex-icon-add-action"></i></a>';
